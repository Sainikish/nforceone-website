      ...
      {items.map((item, i) => (
        <Reveal key={item.title} delay={(i % 4) * 60}>
          <div
            id={item.id}
            className={cn(
              "group h-full rounded-2xl border border-line bg-white p-6 shadow-sm shadow-ink/[0.02] transition-all duration-200 hover:-translate-y-1 hover:border-brand-200 hover:shadow-lg hover:shadow-brand-900/5",
              item.id && "scroll-mt-24"
            )}
          >
            {item.icon && (
